...
<!--                                    <a href="wh_data_entry.php?sorting=enabled"><button class="btn btn-primary" name="abc" id="abc">Enable Sorting</button></a>-->
                            ...
                            <h3 class="page-title row-br-b-wp">Consumption Data Entry - Provincial Level Access <?php echo (!empty($_SESSION['wh_info']['stkname'])?' for "'.$_SESSION['wh_info']['stkname'].'"':'')?></h3>                            
                            <?php
...
<?php if($_SESSION['user_role'] != 76 && $_SESSION['user_role'] != 77) { ?>
                                    <div class="row">
<!--                                        <div class="col-md-3 center red">Please enter Field / Facility Report (Pre requisite) and then District store report. </div>-->
<!--                                        <div class="col-md-9 right"><img src="<?php echo PUBLIC_URL; ?>images/urdu.png"/></div>-->
                                    </div>
<?php } ?>
                                    <div class="portlet box green ">
                                        <div class="portlet-title">
                                            <div class="caption">Provincial / District Stores</div>
                                            ...
                                            <form action="update_facilities_sorting.php" method="post">
                                                <table class="table table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $counter = 1;
                                                        if ($stakeholder == 1 OR $stakeholder == 3190) {
                                                            //data entry url
                                                            $dataEntryURL = 'data_entry_hf_pwd.php';
                                                        } elseif ($user_stakeholder_type == '1' && $_SESSION['user_stakeholder1'] != '3190') {
                                                        ...
                                                        while ($row = mysql_fetch_array($hfResult)) {
                                                            ...
                                                            //check counter
                                                            $ecr = '';
                                                            if(!empty($row['ecr_start_month'])){
                                                                $ecr = '<span class="btn btn-xs blue">ECR</span>';
                                                            }
                                                            ...
                                                            if ($counter % 2 != 0) {
                                                                ...
                                                                echo "<td><span class='wh_name' onClick=\"showReports('$wh_Id', '$load3Months', '$dataEntryURL')\">" . $row['wh_name'] ."  ".$ecr. "</span>";
                                                                echo "<div class=\"whDiv\" id=\"$wh_Id\" style=\"display:none;\"></div>";
                                                                echo "</td>";
                                                            } else if ($counter % 2 == 0) {
                                                                if ($is_sorting == 'enabled') {
        ...
        function showReports(wharehouse_id, load3Months, dataEntryURL)
        {
            ...
            } else
            {
                if (wharehouse_id)
                {
                    ...
                    $.ajax({
                        url: load3Months,
                        data: {wharehouse_id: wharehouse_id, dataEntryURL: dataEntryURL, request_from: 'admin'},
                        type: 'post',
