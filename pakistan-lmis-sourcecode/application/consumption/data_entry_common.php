...
if($_SESSION['user_province1'] == '1' && $_SESSION['user_stakeholder1'] == '7' && $RptDate >= '2022-07-01')
{
   echo '<hr/><span class="note note-info">';
    echo 'NOTE:Data of DOH(Static HF) for the province of Punjab is now automatically fetched from DHIS2, since the month of July 2022. Hence Data Edit is locked in LMIS. ';
...
?>
<a href="refresh_ob.php?Do=<?php echo $_REQUEST['Do']; ?>&month=<?php echo $RptDate;?>&wh=<?php echo $wh_id; ?>">Refresh Opening Balance</a>
<form name="frmF7" id="frmF7" method="post">
    <div class="row">
        <div class="col-md-12">
            <div id="errMsg"></div>
            <table width="100%" align="center" class="table table-bordered">
                <?php if ($_SESSION['user_stakeholder1'] == 2697) { ?>
                <tr>
                    <th rowspan="2" class="text-center">S.No.</th>
                    <th rowspan="2" class="text-center">Article</th>
                    <th rowspan="2" class="text-center">Opening balance</th>
                    <th <?php echo $rowcolspan; ?> class="text-center">Stock Received during reporting period (Pills/Tests/Pcs)</th>                    
                    <th rowspan="2" class="text-center">Quantity consumed during reporting period (pills/Test/PCs)</th>
                    <th colspan="2" class="text-center">Quantity Adjusted  during reporitng period (Expired, Damaged , transferred etc)</th>
                ...
                if ($province_id_session == 3 || $province_id_session == 6) {
                    $item_category_id = "1,5";
                }

                if ($_SESSION['user_role'] == 65 || $_SESSION['user_role'] == 104 || $_SESSION['user_role'] == 102 || $_SESSION['user_role'] == 133) {
	...
	itminfo_tab.itm_name FROM itminfo_tab INNER JOIN stock_batch ON itminfo_tab.itm_id = stock_batch.item_id WHERE stock_batch.wh_id = $sess_wh_id ORDER BY itminfo_tab.itm_name");
                } else {
                    $rsTemp1 = mysql_query("SELECT * FROM `itminfo_tab` WHERE `itm_status`=1 AND `itm_id` IN (SELECT `Stk_item` FROM `stakeholder_item` WHERE `stkid` =$stkid) AND itminfo_tab.itm_category IN ($item_category_id) ORDER BY itm_category,`frmindex`,itm_name ");
                }
                //query
                ...
                while ($rsRow1 = mysql_fetch_array($rsTemp1)) {
                    if (!empty($result_province['prov_id']) && $result_province['prov_id'] != 2 && $result_province['prov_id'] != 1 && $result_province['prov_id'] != 3 && $result_province['prov_id'] != 4 && $result_province['prov_id'] != 5 && $result_province['prov_id'] != 7 && $result_province['prov_id'] != 8) {
                        if ($rsRow1['itm_id'] == 34)
                            continue;
                    }
	...
	stock_sources_data.hf_data_id = $hf_data_id";
                    ...
                    if ($isNewRpt == 1) {
                        ...
                        <tr>
                            ...
                            <td><input class="form-control input-sm text-right" <?php echo (!empty($openOB)) ? 'readonly="readonly"' : ''; ?> autocomplete="off"  type="text" name="FLDOBLA<?php echo $rsRow1['itm_id']; ?>" id="FLDOBLA<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10" value="<?php echo $ob_a; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
                            <?php
                            foreach ($sources as $key => $rowsrc) {
                                ...
                                <td><input class="form-control input-sm text-right FLDSrcs<?php echo $rsRow1['itm_id']; ?>" 
                                    ...
                                    ?> 
                                           autocomplete="off"  type="text" name="FLDSrcs<?php echo $rsRow1['itm_id']; ?>[<?php echo $key; ?>]" id="<?php echo $rowsrc . $rsRow1['itm_id']; ?>" size="8" maxlength="10"  value="<?php echo $sdf; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
                                    <?php
                                }
                                $cb_a += $wh_received;
                                ?>
                            <td><input class="form-control input-sm text-right" <?= (($src_count > 0 || $readonly_for_im) ? 'readonly' : '') ?> autocomplete="off"  type="text" name="FLDRecv<?php echo $rsRow1['itm_id']; ?>" id="FLDRecv<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10"  value="<?php echo $wh_received; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
                            <td><input class="form-control input-sm text-right" autocomplete="off" name="FLDIsuueUP<?php echo $rsRow1['itm_id']; ?>" id="FLDIsuueUP<?php echo $rsRow1['itm_id']; ?>" value="<?php echo $wh_issue_up; ?>" type="text"  size="8" maxlength="10" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
                            <td><input class="form-control input-sm text-right" autocomplete="off" type="text" name="FLDReturnTo<?php echo $rsRow1['itm_id']; ?>" id="FLDReturnTo<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10" value="<?php echo $wh_adja; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');" <?=(($_SESSION['user_stakeholder1'] == 3253)?'readonly':'')?>></td>
                            <td><input class="form-control input-sm text-right" autocomplete="off" type="text" name="FLDUnusable<?php echo $rsRow1['itm_id']; ?>" id="FLDUnusable<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10" value="<?php echo $wh_adjb; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');" <?=(($_SESSION['user_stakeholder1'] == 3253)?'readonly':'')?>></td>
                        ...
                        if ($readonly_for_im) {
                            $wh_received = ((!empty($rcvd_array[$rsRow1['itm_id']])) ? $rcvd_array[$rsRow1['itm_id']] : '0');
                            $rsRow2['closing_balance'] = $rsRow2['opening_balance']+$wh_received+$rsRow2['adjustment_positive']-$rsRow2['issue_balance']-$rsRow2['adjustment_negative'];
                        } else {
                            $wh_received = $rsRow2['received_balance'];
                        }
                        ?>
                        <tr>
                            <td class="text-center" title="<?php echo $rsRow1['itm_id']; ?>"><?php echo $SlNo; ?></td>
                            <td><?php echo $rsRow1['itm_name']; ?>
                                <input type="hidden" name="flitmrec_id[]" value="<?php echo $rsRow1['itm_id']; ?>">
                                <input type="hidden" name="flitm_category[]" value="<?php echo $rsRow1['itm_category']; ?>">
                                <input type="hidden" name="flitmname<?php echo $rsRow1['itm_id']; ?>" value="<?php echo $rsRow1['itm_name']; ?>"></td>
                            <td><input class="form-control input-sm text-right" <?php echo (!empty($openOB)) ? 'readonly="readonly"' : ''; ?> autocomplete="off"  type="text" name="FLDOBLA<?php echo $rsRow1['itm_id']; ?>" id="FLDOBLA<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10" value="<?php echo $rsRow2['opening_balance']; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
                            <?php
                            foreach ($sources as $key => $rowsrc) {
                                ...
                                <td><input class="form-control input-sm text-right FLDSrcs<?php echo $rsRow1['itm_id']; ?>" <?php
                                    ...
                                    ?> autocomplete="off"  type="text" name="FLDSrcs<?php echo $rsRow1['itm_id']; ?>[<?php echo $key; ?>]" id="<?php echo $rowsrc . $rsRow1['itm_id']; ?>" size="8" maxlength="10"  value="<?php echo $this_value; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
        <?php } ?>
                            <td><input class="form-control input-sm text-right"  <?= (($src_count > 0 || $readonly_for_im) ? 'readonly' : '') ?>  autocomplete="off"  type="text" name="FLDRecv<?php echo $rsRow1['itm_id']; ?>" id="FLDRecv<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10"  value="<?php echo $wh_received; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
                            <td><input class="form-control input-sm text-right" autocomplete="off" name="FLDIsuueUP<?php echo $rsRow1['itm_id']; ?>" id="FLDIsuueUP<?php echo $rsRow1['itm_id']; ?>" value="<?php echo $rsRow2['issue_balance']; ?>" type="text" size="8" maxlength="10" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');"></td>
                            <td><input class="form-control input-sm text-right" autocomplete="off"  type="text" name="FLDReturnTo<?php echo $rsRow1['itm_id']; ?>" id="FLDReturnTo<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10" value="<?php echo $rsRow2['adjustment_positive']; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');" <?=(($_SESSION['user_stakeholder1'] == 3253)?'readonly':'')?>></td>
                            <td><input class="form-control input-sm text-right" autocomplete="off"  type="text" name="FLDUnusable<?php echo $rsRow1['itm_id']; ?>" id="FLDUnusable<?php echo $rsRow1['itm_id']; ?>" size="8" maxlength="10" value="<?php echo $rsRow2['adjustment_negative']; ?>" onfocusout="cal_balance('<?php echo $rsRow1['itm_id']; ?>');" <?=(($_SESSION['user_stakeholder1'] == 3253)?'readonly':'')?>></td>
    ...
    if ($isNewRpt != 2) {
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                if($_SESSION['user_province1'] == '1' && $_SESSION['user_stakeholder1'] == '7' && $RptDate >= '2022-07-01')
                {
                    ///// dont provide save button.
                    echo 'Data is not editable .';
                }
                else{
                ?>
