...
FROM itminfo_tab INNER JOIN stock_batch ON itminfo_tab.itm_id = stock_batch.item_id WHERE stock_batch.wh_id = $wh_id ORDER BY itminfo_tab.itm_name ");
} else {
    $rsTemp1 = mysql_query("SELECT * FROM `itminfo_tab` WHERE `itm_status`=1 AND `itm_id` IN (SELECT `Stk_item` FROM `stakeholder_item` WHERE `stkid` =$stkid) AND itminfo_tab.itm_category IN ($item_category_id) ORDER BY itminfo_tab.itm_name ");
}
?>
