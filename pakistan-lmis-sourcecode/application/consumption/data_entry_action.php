...
if ($_POST['ActionType'] == 'Add') {
    ...
    foreach ($postedArray as $val) {
        ...
        $adjCmnts = isset($_POST['FLDCMNTS' . $itemid[1]]) ? $_POST['FLDCMNTS' . $itemid[1]] : '';
    ...
    if (strlen($error) > 0) {
        ...
        foreach ($postedArray as $val) {
            ...
            $FLDCMNTS = isset($_POST['FLDCMNTS' . $itemid[1]]) ? $_POST['FLDCMNTS' . $itemid[1]] : 0;
            //Getting wh-id
            $wh_id = $_POST['wh_id'];
            $itemid = $val;
            //Getting year
            ...
            if ($data['num'] == 0) {
				...
				wh_received,wh_issue_up,wh_cbl_a,wh_adja,wh_adjb,RptDate,add_date,last_update,ip_address,created_by,comments)  Values(		
				" . $_POST['mm'] . ",
				" . $_POST['yy'] . ",							
				'" . $val . "',
				" . $_POST['wh_id'] . ",
