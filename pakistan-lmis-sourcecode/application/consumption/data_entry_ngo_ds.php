...
// this is for data entry of district stores of NGOs only. 
// supposed to be used at provincial level or district level.
...
$stks_with_prov_to_dist_flow=array('163','3375','3364','3320','3190','154','4','6','3404','3421');
...
//                    exit;
                    ...
                    // Stakeholder 6 = MSS
                    // Stakeholder 9 = PPHI
                    if ($qryLvl['lvl'] == 3 && $_SESSION['user_stakeholder_type'] == 1) {
                        $readonly = '';
                    } else if ($qryLvl['lvl'] == 2 && $_SESSION['user_stakeholder_type'] == 1) {
                    ...
                    ////////////////////////////////////////////////////////////////
                    ///// As per email titled: 'District Store's Received Column is required to be "automated"'
                    ///// And linked JIRA Ticket number LMIS-3338 & LSD-6499, and discussion with Aqeel sb over phone.
                    ///// PWD Sindh has requested to block the recieving column of district stores for the district level users,
                    ///// while to keep it open for district stores for provincial users.
                    //////////////////////////////////////////////////////////////////
                    
                    
                    if (in_array($_SESSION['user_stakeholder1'], $stks_with_prov_to_dist_flow))
                    {
                    ...
                    //echo $wh_id.','.$readonly.' Stk :'.$_SESSION['user_stakeholder1'];exit;
                    ...
                    ?>
                <a href="refresh_ob_dist_store.php?Do=<?php echo $_REQUEST['Do']; ?>&month=<?php echo $RptDate;?>&wh=<?php echo $wh_id; ?>">Refresh District Store Opening Balance</a>
                            ...
                            <div class="col-md-12">
                                ...
                                <table class="table table-bordered">
                                    <tr>
                                        ...
                                        if($wh_lvl == '2'){
                                            $t1 = ' ( From CW&S ) ';
                                            $t2 = ' ( To District Store ) ';
                                        }
                                        if($wh_lvl == '3'){
                                            $t1 = ' ( From Provincial Store ) ';
                                            $t2 = ' ( To SDP / HF ) ';
                                        }
                                        ?>
                                        
                                        <th <?php echo $rowcolspan; ?> class="text-center">Received <?php echo $t1 ?></th>
                                        <th rowspan="2" class="text-center">Issued <?php echo $t2 ?></th>
                                ...
                                AND tbl_wh_data_draft.wh_id = $wh_id";
                                    ...
                                    $rsTemp1 = mysql_query("SELECT * FROM `itminfo_tab` WHERE `itm_status`=1 AND `itm_id` IN (SELECT `Stk_item` FROM `stakeholder_item` where stkid = '$stkid') AND itminfo_tab.itm_category = 1 ORDER BY itm_category,`frmindex`,itm_name");
                                   //echo "SELECT * FROM `itminfo_tab` WHERE `itm_status`=1 AND `itm_id` IN (SELECT `Stk_item` FROM `stakeholder_item` where stkid = '$stkid') AND itminfo_tab.itm_category = 1 ORDER BY itm_category,`frmindex`,itm_name";
                                    //exit;
                                    $SlNo = 1;
                                    $fldIndex = 0;
                                    while ($rsRow1 = mysql_fetch_array($rsTemp1)) {
					if(!empty($result_province['prov_id']) && !($result_province['prov_id'] == 2 || $result_province['prov_id'] == 3   || $_SESSION['user_stakeholder'] == 4 )){
                                            if($rsRow1['itm_id'] == 34)
                                                continue;
                                        }
                                        ...
                                        $item_int = $rsRow1['itm_id'];
                                        
//                                        $qry_cf = "SELECT REPUpdateCarryForward('$RptDate','$item_char', $wh_id) FROM DUAL";
//                                        mysql_query($qry_cf);
                                        ...
                                        if ($qryLvl['lvl'] == 2){
                                            ...
                                            require 'get_ds_receiving_ngo.php';
                                            require 'get_cwh_store_issuance_ngo.php';
                                        }
                                        elseif ($qryLvl['lvl'] == 3){
                                            
                                            require 'get_hf_receiving_ngo.php';
                                        }
...
FROM
	stock_sources_data
INNER JOIN tbl_wh_data ON stock_sources_data.wh_data_id = tbl_wh_data.w_id
WHERE
	stock_sources_data.wh_data_id = $hf_data_id";                                        
                                        //result
                                        $rsTemp4 = mysql_query($qryd);
                                        $sources_data = array();
                                        if(!empty($rsTemp4)){
                                            while ($rsRow4 = mysql_fetch_array($rsTemp4)) {
                                                $itemid1 = explode('-', $rsRow4['item_id']);
                                                $sources_data[$rsRow4['stock_sources_id']][$itemid1[1]] = $rsRow4['received'];
                                            }
                                        }

                                        //print_r($sources_data);
                                        ...
                                        if ($isNewRpt == 1) {
                                            ...
                                            if ($issuance_flag == 'TRUE') {
                                                $wh_issue_up = $district_issuance;
                                            }
                                            if ($cwh_issuance_flag == 'TRUE') {
                                                $wh_received = $cwh_issuance;
                                            }
                                            ...
                                            <tr>
                                                ...
                                                foreach ($sources as $key => $rowsrc) {
                                                    $sdf = 0;
                                                    ?>
                                                    <td><input class="form-control input-sm text-right FLDSrcs<?php echo $itemid[1]; ?>" <?php
                                                        if ($key == 130) {
                                                            echo $isReadOnly . $style;
                                                        }
                                                        ?> autocomplete="off"  type="text" name="FLDSrcs<?php echo $itemid[1]; ?>[<?php echo $key; ?>]" id="<?php echo $rowsrc . $itemid[1]; ?>" size="8" maxlength="10"  value="<?php echo $sdf; ?>" onKeyUp="cal_balance('<?php echo $itemid[1]; ?>');"></td>
                                                    <?php } ?>
                                                <td><input class="form-control input-sm text-right" <?= (($src_count > 0) ? 'readonly' : '') ?> autocomplete="off"  type="text" name="FLDRecv<?php echo $itemid[1]; ?>" id="FLDRecv<?php echo $itemid[1]; ?>" size="8" maxlength="10"  value="<?php echo $wh_received; ?>" onKeyUp="cal_balance('<?php echo $itemid[1]; ?>');" <?php echo $readonly; ?>></td>
                                                <td><input class="form-control input-sm text-right" autocomplete="off" name="FLDIsuueUP<?php echo $itemid[1]; ?>" id="FLDIsuueUP<?php echo $itemid[1]; ?>" value="<?php echo $wh_issue_up; ?>" type="text"  size="8" maxlength="10" onKeyUp="cal_balance('<?php echo $itemid[1]; ?>');" readonly></td>
                                        ...
                                        // If oone of the 3 months
                                            ...
                                            if ($issuance_flag == 'TRUE') {
                                                $rsRow2['wh_issue_up'] = $district_issuance;
//                                                echo 'EX'.$district_issuance;
//                                                echo '<pre>';
//                                                print_r($rsRow2 );
//                                                exit;
                                            }
                                            if (@$cwh_issuance_flag == 'TRUE') {
                                                $rsRow2['wh_received'] = $cwh_issuance;
                                            }
                                            ?>
                                            <tr>
                                                <td class="text-center"><?php echo $SlNo; ?></td>
                                                ...
                                                <?php
                                                foreach ($sources as $key => $rowsrc) {
                                                    $this_value = (!empty($sources_data[$key][$itemid[1]]) ? $sources_data[$key][$itemid[1]] : '0');
                                                    ?>
                                                    <td><input class="form-control input-sm text-right FLDSrcs<?php echo $itemid[1]; ?>" <?php
                                    if ($key == 130) {
                                        echo $isReadOnly . $style;
                                    }
                                                    ?> autocomplete="off"  type="text" name="FLDSrcs<?php echo $itemid[1]; ?>[<?php echo $key; ?>]" id="<?php echo $rowsrc . $itemid[1]; ?>" size="8" maxlength="10"  value="<?php echo $this_value; ?>" onKeyUp="cal_balance('<?php echo $itemid[1]; ?>');"></td>
                                                           <?php } ?>
                                                <td><input class="form-control input-sm text-right" <?= (($src_count > 0) ? 'readonly' : '') ?> autocomplete="off"  type="text" name="FLDRecv<?php echo $itemid[1]; ?>" id="FLDRecv<?php echo $itemid[1]; ?>" size="8" maxlength="10"  value="<?php echo $rsRow2['wh_received']; ?>" onKeyUp="cal_balance('<?php echo $itemid[1]; ?>');" <?php echo $readonly; ?>></td>
                                                <td><input class="form-control input-sm text-right" autocomplete="off" name="FLDIsuueUP<?php echo $itemid[1]; ?>" id="FLDIsuueUP<?php echo $itemid[1]; ?>" value="<?php echo $rsRow2['wh_issue_up']; ?>" type="text" size="8" maxlength="10" onKeyUp="cal_balance('<?php echo $itemid[1]; ?>');" readonly></td>
    ...
    <script>
                                        ...
                                        function autoSave()
                                        {
                                           // dont perform any action for ngos
                                        }
