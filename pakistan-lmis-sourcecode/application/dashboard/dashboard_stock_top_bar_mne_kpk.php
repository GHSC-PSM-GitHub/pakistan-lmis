...
AND tbl_locations.PkLocID  IN (" . (!empty($dist_arr_request) ? $dist : $default_dist) . ")  
AND tbl_warehouse.stkid IN (" . (!empty($stk_arr) ? $stk : $default_stk) . ") AND
tbl_warehouse.is_active = 1 AND
stakeholder.lvl > 2
ORDER BY
tbl_locations.LocName ASC";

//echo $qry_total_districts;exit;
...
tbl_warehouse.prov_id = $province 
    AND tbl_warehouse.dist_id IN (" . (!empty($dist_arr_request) ? $dist : $default_dist) . ")
AND tbl_warehouse.stkid IN (" . (!empty($stk_arr) ? $stk : $default_stk) . ") AND
tbl_hf_data.reporting_date = '$from_date'";
//echo $qry_rep_dist;exit;
...
AND summary_district.stakeholder_id IN (" . (!empty($stk_arr) ? $stk : $default_stk) . ") AND
    ...
    AND summary_district.province_id = $province
        AND summary_district.district_id IN (" . (!empty($dist_arr_request) ? $dist : $default_dist) . ")
GROUP BY summary_district.item_id";
//echo $qry_so_dist;exit;
...
<div class="<?php echo $col_1; ?> center">
    <div class="dashboard-stat red-pink" style="height:100px;background-color: #81ce4b;">
        <div class="visual"><i class="fa fa-<?php if ($province == 3) echo "kpk";else if ($province == 4) echo"baloochistan";
else echo lcfirst($prov_name); ?>"></i></div>
        <div class="details">
