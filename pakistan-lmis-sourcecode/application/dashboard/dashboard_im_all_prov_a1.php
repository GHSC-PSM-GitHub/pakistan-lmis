...
<div class="widget widget-tabs" style="border:0px;">
    <div class="widget-body" id="a1" style='background-color: white;'>
        ...
        <h3 style="text-align: center;"><?php echo "District Stores Month wise Consumption/Issue Trend (Drilldown)" ?></h3>
...
foreach ($wh_name as $key => $value) {
    ...
    foreach ($reporting_array as $date => $val) {
        if (!isset($stock_arr[$key][$date])) {
            ...
            $xmlstore .= '    <set  value="' . $stock_arr[$key][$date] . '" link="P-detailsWin,width=900,height=600,toolbar=no,scrollbars=yes, resizable=no-dashboard_im_all_prov_a1_table.php?prov_sel='.$province.'&product='.$product.'&wh_id='.$key.'&wh_name='.$value.'&from_date='.$from_date.'&to_date='.$to_date.'&search_type='.$search_type.'&district='.implode($district,',').'" />';
        }
    }
    $xmlstore .= '  </dataset>';
}
...
echo renderChart(PUBLIC_URL . "FusionCharts/Charts/MSCombiDY2D.swf", "", $xmlstore, 'c_chart', '100%', 300, false, false);
?>
