        ...
        <div class="page-content-wrapper">
            <div class="page-content">
                ...
                <div class="container-fluid">
                                                                       ...
                                                                       <option value="">All</option>
                                                                        <?php
                                                                            ...
                                                                            if(!empty($_SESSION['user_level']) && $_SESSION['user_level'] >= 2 &&!empty($_SESSION['user_province1']) && $_SESSION['user_province1'] !=10)
                                                                              ...
                                                                              if($_SESSION['user_id'] == 8913 OR $_SESSION['user_id'] == 8900)  {
                                                                                  $queryprov = "SELECT
                                                    ...
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            <label class="control-label">Select Type</label>
                                                                <select name="fd" id="fd"  onchange="showDistricts()"  class="form-control input-sm">
                                                                       <option value="">All</option>
                                                                       <option value="2" <?php if(isset($_POST['fd']) && $_POST['fd'] != "") { echo 'selected';} ?>>Focused Districts</option>
                                                                </select>
                                                        </div>
                                                    </div>
                                                </td>
                    ...
                    <?php
                      $pop = 'onclick="window.open(\'ajax_fp2020_irmnch.php?prov_name=' . $prov_name . '&province='.$selProv.'&from_date='.$fromDate.'&stakeholder='.$funding_source.'&product='.implode($itm_arr_request,',').'&stk_name='.$stk_name.'\',\'_blank\',\'scrollbars=1,width=840,height=595\')"';
                      ...
                      <div class="row hide_divs" >
                            <div class="col-md-12">
                                <h3 class="page-title row-br-b-wp center"> <?=(!empty($prov_name)?$prov_name:'All Provinces')?><?=(!empty($district)?' - '.$dist_name:'')?> - Executive Dashboard - <?php echo date('M-Y',strtotime($fromDate)); ?>    
                                    ...
                                    <a class="pull-right" style="align:right;margin-right:2%;cursor: pointer;<?php if($selProv!=1) echo 'display:none;';?>"  <?php echo $pop;?>>IRMNCH Msd Stock</a>
                                </h3>
    ...
    <script type="text/javascript">
		...
		function showDistricts() {
			$.ajax({
				type: "POST",
				url: '<?php echo APP_URL; ?>dashboard/ajax.php',
				data: {lvl: 3, type: $('#fd').val(), all_option:'yes', prov_id: $('#province').val()},
				success: function(data) {
					$("#td_dist").html(data);
				}
			});
