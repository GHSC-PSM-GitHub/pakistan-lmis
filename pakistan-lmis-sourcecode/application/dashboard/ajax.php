...
//query
    ...
    $qry = "SELECT
			...
			FROM
				tbl_locations
                              INNER JOIN flood_affected_districts ON tbl_locations.PkLocID = flood_affected_districts.dist_id 
			WHERE
				tbl_locations.ParentID = $prov_id
			ORDER BY
				tbl_locations.LocName ASC";
...
if (isset($_POST['required_value']) && $_POST['required_value']=='funding_sources') {
    ...
    $qry = "SELECT
                ...
                WHERE
                funding_stk_prov.province_id = $prov_id and (tbl_warehouse.prov_id is null OR tbl_warehouse.prov_id = $prov_id OR tbl_warehouse.prov_id=10)";
    //query result
    $qryRes = mysql_query($qry);
    //fetch results
