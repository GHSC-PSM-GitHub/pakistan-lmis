...
<div class="widget widget-tabs" style="border:0px;">
    <div class="widget-body" id="a4" style='background-color: white;'>
        ...
        <h3 style="text-align: center;"><?php echo "District Stores Month wise AMC Trend " ?></h3>
...
//                print_r($qry_amc);
                        ...
                        while ($row = mysql_fetch_assoc($result)) {
                            $stock_arr[$row['wh_id']][$date] += $row['AMC'];
...
//        exit;
        ...
        foreach ($wh_name as $key => $value) {
            ...
            $xmlstore .= '    <set  value="' . $stock_arr[$key][$date] . '" link="P-detailsWin,width=900,height=600,toolbar=no,scrollbars=yes, resizable=no-dashboard_im_all_prov_a4_table.php?prov_sel='.$province.'&product='.$product.'&wh_id='.$key.'&wh_name='.$value.'&from_date='.$from_date.'&to_date='.$to_date.'&search_type='.$search_type.'&district='.implode($district,',').'" />';
                }
            }
            $xmlstore .= '  </dataset>';
        }
        ...
        echo renderChart(PUBLIC_URL . "FusionCharts/Charts/MSCombiDY2D.swf", "", $xmlstore, 'amc_chart', '100%', 300, false, false);
        ?>
