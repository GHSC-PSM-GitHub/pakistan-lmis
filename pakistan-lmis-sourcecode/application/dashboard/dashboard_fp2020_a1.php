...
$default_funding_source = '6891,9079';
...
if (!empty($funding_source))
    $qry_c .= "   funding_stk_prov.funding_source_id IN ($funding_source) ";
else
    $qry_c .= "   funding_stk_prov.funding_source_id IN ($default_funding_source) ";
...
$qry_3 .= " AND stock_batch.funding_source IN  (" . ((!empty($funding_source)) ? $funding_source : $default_funding_source) . ") ";
$qry_3 .= " GROUP BY
			itminfo_tab.itm_id
		ORDER BY
...
$qry_6 = "
            ...
            SELECT
                ...
                shipments.`status`,
                shipments.`pk_id` as shipment_id
            FROM
                    shipments
...
while ($row_6 = mysql_fetch_assoc($res_6)) {
    $pipeline_detail_arr[$row_6['itm_id']][$row_6['shipment_id']] = $row_6;
    ...
    $pipeline_arr[$row_6['itm_id']][$from_date] += $s_q;
}
//separate qry for received qty of shipment
...
$qry_6 = "
            ...
            WHERE
                ...
                AND shipments.status NOT IN ('Cancelled','Received') AND 
                    shipments.wh_id = 123
                AND tbl_stock_master.TranDate <= '$f2_date' 
...
$res_6 = mysql_query($qry_6);
while ($row_6 = mysql_fetch_assoc($res_6)) {
    $pipeline_detail_arr[$row_6['itm_id']][$row_6['shipment_id']]['received_qty'] = $row_6['received_qty'];
    $r_q = isset($row_6['received_qty']) ? $row_6['received_qty'] : '0';
    if (empty($pipeline_arr[$row_6['itm_id']][$from_date]))
        $pipeline_arr[$row_6['itm_id']][$from_date] = 0;
    $pipeline_arr[$row_6['itm_id']][$from_date] -= $r_q;
}
...
//                        '<br/><br/><br/>USAID Supported stock allocated by PPW
                   ' </td>
                   ';
...
//                    '<br/><br/><br/><br/>'. number_format($national_q).
                        '</td>
