...
//    $xmlDistrict .= "<cell><![CDATA[" . $row['distName'] . '<a href="../developer_reports/district_data_fix.php?"><img style="float: right" height="15" src="https://img.icons8.com/ios-glyphs/30/FA5252/refresh--v1.png" alt="refresh--v1"/></a>' . "]]></cell>";
    $xmlDistrict .= "<cell><![CDATA[" . $row['distName'] . '<img onclick="refresh_district('.$row['distId'].', '.$row['provId'].')" style="float: right" height="15" src="https://img.icons8.com/ios-glyphs/30/FA5252/refresh--v1.png" alt="refresh--v1"/>' . "]]></cell>";
...
<script>
    function refresh_district(district, province){
         
        
        // alert(district);
        let sel_year = <?= $sel_year ?>;
        let sel_month = <?= $sel_month ?>;
        if (sel_month < 10){
            sel_month = '0' + sel_month;
        }
        let sel_stk = <?= $sel_stk ?>;
        let sel_prov = province;
        let sel_item = '<?= $sel_item ?>';

        $.ajax({
            type: "POST",
            url: '<?php echo APP_URL; ?>data_fix/district_data_fix2.php',
            data: {
                date: sel_year + '-' + sel_month + '-01',
                item: sel_item,
                dist: district,
                stk: sel_stk,
                prov: sel_prov
...
<div class="spanner">
    ...
    <p>Recalculating data. Please wait</p>
</div>
<!--CSS-->
<style>
    .spanner{
