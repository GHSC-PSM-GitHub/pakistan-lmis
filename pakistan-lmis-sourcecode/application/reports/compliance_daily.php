...
if (isset($_REQUEST['submit'])) {
    ...
    $selStk = !empty($_REQUEST['stk_sel']) ? $_REQUEST['stk_sel'] : '1';
    $selPro = !empty($_REQUEST['prov_sel']) ? $_REQUEST['prov_sel'] : '1';
    $selDist = !empty($_REQUEST['district']) ? $_REQUEST['district'] : '';
    
    $months_list=array();
   
    $start_date       =   $selYear.'-'.$selMonth.'-01';
    $last_date = date("Y-m-t", strtotime($selYear ."-".$selMonth."-01"));
    for ( $i = strtotime($start_date.' 00:00:00'); $i <= strtotime($last_date.' 00:00:00'); $i = $i + 86400 ) {
         $months_list[]  =  date( 'Y-m-d',  $i  );
    }
//    krsort($months_list);
                        ...
                        <div class="widget" data-toggle="collapse-widget">
                            ...
                            <?php
									...
									AND stakeholder.stk_type_id IN (0, 1, 4)
                                                                        AND stakeholder.stkid in (7,376)
									ORDER BY
...
$where_clause .=" AND tbl_warehouse.dist_id =$selDist ";
        ...
        if(!empty($itm))             
            $w_clause .= " AND itminfo_tab.itm_id in (".$itm.")  ";    
                ...
                $q_reporting  = "SELECT
                                        ...
                                        tbl_locations.LocName,tbl_hf_data_daily.reporting_date,
                                        SUM(tbl_hf_data_daily.issue_balance) consumptionqty
                                FROM
                                        tbl_warehouse
        ...
        $q_reporting .= "
                                ...
                                INNER JOIN stakeholder ON tbl_warehouse.stkofficeid = stakeholder.stkid
                                INNER JOIN tbl_hf_data_daily ON tbl_warehouse.wh_id = tbl_hf_data_daily.warehouse_id
                                INNER JOIN tbl_locations ON tbl_warehouse.dist_id = tbl_locations.PkLocID ";
        if($selPro == 3 && $selStk ==7 )
        $q_reporting .= "   INNER JOIN itminfo_tab ON tbl_hf_data_daily.item_id = itminfo_tab.itm_id ";
        $q_reporting .= "
                                WHERE
                                     stakeholder.lvl = 7
                                     /*AND tbl_warehouse.hf_type_id NOT IN (5, 2, 3, 9, 6, 7, 8, 12, 10, 11)*/
                                     AND tbl_warehouse.reporting_start_month <=tbl_hf_data_daily.reporting_date
                                AND tbl_hf_data_daily.reporting_date BETWEEN '".$start_date."' and '".$last_date."'
                                ";
        if($selPro == 3 && $selStk ==7 )
        ...
        $q_reporting .= "
                                ...
                                GROUP BY
                                        tbl_warehouse.wh_id,
                                        tbl_hf_data_daily.reporting_date
                ...
                <div class="row">
                    <div class="col-md-12">
                        <table width="100%" cellpadding="0" cellspacing="0" id="myTable1" class="table table-bordered table-condensed">
                             <tr>
                                 <td align="center" width="90%"   ><h3 class="text-info">Compliance Report Health Facility Wise - <?=$province_name?> - <?=$dist_name?> - <?=$stk_name?></h3><h4><?=$month_name?></h4></td>
                        ...
                        <div>
                            ...
                            <tr class=" bg-blue-madison">
                                ...
                                foreach($months_list as $k => $month ){
                                    echo '<td align="center">'.date('d',strtotime($month)).'</td>';
                                }
                                ?>
                            </tr>
                            <?php
                                ...
                                foreach($total_sdps as $wh_id => $wh_name ){
                                    ...
                                    foreach($months_list as $k => $month ){
                                        ...
                                        if( !empty($reporting_wh_arr[$wh_id][$month])   ){
                                            ...
                                            $display .= '<span onclick="window.open(\'wh_info_daily.php?whId='.$wh_id.'&month='.$m1.'&year='.$y1.'&date='.$d1.'\', \'_blank\', \'scrollbars=1,width=900,height=500\')">';
