...
if (isset($_REQUEST['param'])) {
    ...
    if ($ind == 1) {
        ...
        $qry = "SELECT
				...
				AND tbl_wh_data.RptDate BETWEEN '" . $year . "'
				AND '" . $month . "'
...
while ($row = mysql_fetch_array($qryRes)) {
    ...
    $xmlstore .= "<cell>" . (($ind == 3) ? number_format($val) : number_format($val)) . "</cell>";
    $xmlstore .= "</row>";
}
//xml row
$xmlstore .= "<row>";
...
//include reports_includes
    ...
    <script>
        ...
        function doInitGrid() {
            ...
            mygrid.setHeader("<div style='text-align:center;'>District wise <?php echo $indicator; ?> for Province '<?php echo $province; ?>' Stakeholder '<?php echo $stakeholder; ?>' Product '<?php echo $itmName; ?>' (<?php echo 'From : '.date("m/d/Y", strtotime($year));echo ' TO : '.date("m/d/Y", strtotime($month)); ?>),#cspan,#cspan");
