...
if (isset($_POST['submit'])) {
    ...
    if (empty($districtId) || $districtId == null || $districtId == 'all') {
...
//if submitted
                if (isset($_POST['submit'])) {
                    ...
                    $qryResCypFactor = mysql_query($qry_cypfactor);
                    if (mysql_num_rows($qryResCypFactor) > 0) {
                        while ($row = mysql_fetch_array($qryResCypFactor)) {
                            $cyp_array[$row['province_id']][$row['stakeholder_id']][$row['item_id']] = $row['CYPFactor'];
                        }
                    }
//                    echo "<pre>";
//                    print_r($cyp_array);exit;
                    $qry = "SELECT
A.hf_type,
	A.wh_id,
	A.wh_name,
                        ...
                        <div class="col-md-12" style="overflow:auto;">
                            ...
                            while ($row = mysql_fetch_array($qryRes)) {
                                if ($row['itm_category'] == 2) {
                                    $row['CYP'] = $cyp_array[$selProv][$stakeholder][$row['itm_id']] * $row['Performed'];
                                } else {
                                    $row['CYP'] = $cyp_array[$selProv][$stakeholder][$row['itm_id']] * $row['issue_balance'];
                                }
