...
`From`.wh_name as recv_from,
`To`.wh_name as issue_to,
...
INNER JOIN tbl_warehouse AS `From` ON tbl_stock_master.WHIDFrom = `From`.wh_id
INNER JOIN tbl_warehouse AS `To` ON tbl_stock_master.WHIDTo = `To`.wh_id 
WHERE
DATE_FORMAT(
				tbl_stock_master.TranDate,
				'%Y-%m-%d'
...
//        print_r($lastModified);exit;
        ...
        //xml
        $xmlstore .= "<row>";
        $xmlstore .= "<cell>" . $row_xmlw['last_update'] . "</cell>";
        $xmlstore .= "<cell>" . $row_xmlw['itm_name'] . "</cell>";
        $xmlstore .= "<cell>" . $row_xmlw['recv_from'] . "</cell>";
        $xmlstore .= "<cell>" . $row_xmlw['issue_to'] . "</cell>";
        //wh_obl_a
        if ($lvl == 3) {
        ...
        $xmlstore .= "<cell>" . $row_xmlw['batch_expiry'] . "</cell>";
        
        $d1 = new DateTime($row_xmlw['batch_expiry']);
        $d2 = new DateTime($row_xmlw['last_update']);
        $interval = $d1->diff($d2);
        $seconds = $interval->s; //45
        $minutes = $interval->i; //23
        $hours   = $interval->h; //8
        $days    = $interval->d; //21
        $months  = $interval->m; //4
        $years   = $interval->y; //1
        
        $xmlstore .= "<cell>" . (($years * 12)+$months).' months '.$days.' days' . "</cell>";
