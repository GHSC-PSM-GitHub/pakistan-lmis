...
if (!empty($province))
    $where_clause .= " AND medicine_avg_stock_out_summary.province_id in (" . $province . ")  ";
if (!empty($stk))
    $where_clause .= " AND medicine_avg_stock_out_summary.stakeholder_id in (" . $stk . ")  ";
?>
                                                ...
                                                <td class="col-md-2">
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            ...
                                                            <select name="district[]" id="district" class="multiselect-ui form-control input-sm" multiple>
                                                                <?php
                                                                $queryDist = "SELECT
                                                                    tbl_locations.LocName,
                                                                    tbl_locations.PkLocID
                                                                    FROM tbl_locations
                                                                    where tbl_locations.ParentID in (3,6)";
                                                                //query result
                                                                $rsDist = mysql_query($queryDist) or die();
                                                                $dist_name = array();
                                                                while ($rowDist = mysql_fetch_array($rsDist)) {
                                                                    if (!isset($_REQUEST['district'])) {
                                                                        if (!empty($rowDist['PkLocID']) && $rowDist['PkLocID'] != 'All') {
                                                                            $dist_arr_request[] = $rowDist['PkLocID'];
                                                                            $sel = "selected='selected'";
                                                                            $dist_name[$rowDist['PkLocID']] = $rowDist['LocName'];
                                                                        } else {
                                                                            $sel = "";
                                                                        }
                                                                    }
                                                                        if (in_array($rowDist['PkLocID'], $dist_arr_request)) {
                                                                            $sel = "selected='selected'";
                                                                            $dist_name[$rowDist['PkLocID']] = $rowDist['LocName'];
                                                                        } else {
                                                                            $sel = "";
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <select  required name="stakeholder[]" id="stakeholder" class="multiselect-ui form-control input-sm" multiple>
                                                                ...
                                                                while ($rowprov = mysql_fetch_array($rsprov)) {
                                                                    $sel = in_array($rowprov['stkid'], $stk_arr)?'selected':'';
                                                                ?>
                                                    ...
                                                    <div class="col-md-12">
                                                        <div class="form-group">
                                                            ...
                                                            <?php
                                                                $queryprov = "SELECT
                                                                                itminfo_tab.itm_id,
                                                                                itminfo_tab.generic_updated
                                                                                FROM
                                                                                    itminfo_tab
                                                                                WHERE
                                                                                    itminfo_tab.itm_id IN (48, 55, 51, 1107, 52)
                                                                                ORDER BY
                                                                                    itminfo_tab.generic_updated";
                                                                //query result
                                                                $rsprov = mysql_query($queryprov) or die();
                                                                while ($rowprov = mysql_fetch_array($rsprov)) {
                                                                    $sel = in_array($rowprov['itm_id'], $itm_arr_request)?'selected':'';
                                                            ?>
                                                                    <option value="<?php echo $rowprov['itm_id']; ?>" <?php echo $sel;?>><?php echo $rowprov['generic_updated'];?></option>
                                                            <?php
                                                                }
                                                            ?>
                                                            </select>
                ...
                if (isset($_REQUEST['submit_btn'])) {
                    ...
                    /*$qry_1 = "  
                                        ...
                                        stakeholder_item.stk_item
                                            ...
                                            $qry_1 = "  
                                            SELECT
medicine_avg_stock_out_summary.province_id AS prov_id,
medicine_avg_stock_out_summary.product_id AS itm,
medicine_avg_stock_out_summary.total_facilities AS totalWH,
medicine_avg_stock_out_summary.reporting_month,
medicine_avg_stock_out_summary.reported_facilities,
medicine_avg_stock_out_summary.disabled_facilities,
medicine_avg_stock_out_summary.dormant_facilities,
medicine_avg_stock_out_summary.so_facilities,
medicine_avg_stock_out_summary.province_name LocName,
medicine_avg_stock_out_summary.product_name
FROM
	medicine_avg_stock_out_summary
WHERE
medicine_avg_stock_out_summary.reporting_month BETWEEN '" . $from_date . "' AND '" . $to_date . "'
$where_clause
AND medicine_avg_stock_out_summary.product_id IN ($itm)";
                    //echo $qry_1;exit;
