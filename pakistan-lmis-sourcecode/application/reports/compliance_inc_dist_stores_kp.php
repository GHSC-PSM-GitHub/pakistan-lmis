...
$filter='';
if ($rep_level==3){
    $filter=" AND tbl_warehouse.prov_id=".$_SESSION['user_province'];

    if ($_SESSION['user_province'] == 3){
        $filter = "AND tbl_warehouse.prov_id IN (3,6)";
    }
}
//print_r($selPro);exit;
...
//            echo $qry_1;exit;
            ...
            {
                $provinces[$row_1['wh_id']] = $row_1['wh_name'];
                
                $total_dist[$row_1['wh_id']]=$row_1['total_districts'];
...
//            echo '<pre>';print_r($total_dist);exit;
            ...
            //
            $prov_filter = "AND tbl_warehouse.prov_id = " . $_SESSION['user_province'];
            if ($_SESSION['user_province'] == 3){
                $prov_filter = "AND tbl_warehouse.prov_id IN (3,6)";
            }
            $qry = "SELECT
                ...
                AND tbl_wh_data.RptDate BETWEEN '$start_date' AND '$last_date'
...
//            exit;
            ...
            {
                $reporting_wh_arr[$row['wh_id']][$row['stkid']][$row['RptDate']]= $row['reported'];
                if(empty($reporting_prov_wise[$row['wh_id']][$row['RptDate']])) $reporting_prov_wise[$row['wh_id']][$row['RptDate']]=0;
                $reporting_prov_wise[$row['wh_id']][$row['RptDate']] += $row['reported'];
                ...
                <div class="row" id="div_main">
                    <div class="col-md-12" id="div_main2">
                        <table width="100%" cellpadding="0" cellspacing="0" id="myTable" class="table table-bordered table-condensed">
                             <tr>
                                 <td align="center" width="95%"   ><h3 class="text-info">Compliance Report - District - <?=$stk_name?></h3></td>
...
//
    ...
    {
        if($prov=='all')continue;
        $chart_data .= ' <dataset seriesname="'.$provinces[$prov].'" '.(($temp_count>1) ? ' ':'').'>';
        foreach($months_list as $k => $month)
        {   
            $reported =0;
            if(!empty($reporting_prov_wise[$prov][$month]))
