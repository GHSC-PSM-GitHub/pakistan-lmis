    ...
    <div class="page-container">
        ...
        <div class="page-content-wrapper">
            <div class="page-content">
                <div class="row">
                    <div class="col-md-12">
                        <h3 class="page-title row-br-b-wp">FP Stock Status</h3>
...
//print_r($hf_cb);
//exit;
                    //getting issuance of DHQ hospital 
                    $qry_dhq_issuance = "SELECT
                                            ...
                                            stock_batch.item_id,
                                            tbl_warehouse.dist_id";
//                    echo $qry_dhq_issuance;exit;
                    // where reporting date between $from and $to
                    // and dist_id in ()
                    $dhq_issuance = array();
                    $qryResDhqIssuance = mysql_query($qry_dhq_issuance);
                    //fetch data from qry
                    if (mysql_num_rows(mysql_query($qry_dhq_issuance)) > 0) {
...
//
//echo $qry_dhq_issuance;
// echo '<pre>';
//                   print_r($dhq_issuance);
//                   exit;
                    //getting closing of dhq 
                    $qry_dhq_cb = "SELECT
                                                Sum(tbl_stock_detail.Qty) AS wh_cbl_a, 
                                                stock_batch.item_id, 
                                                tbl_warehouse.dist_id
                                            ...
                                            stock_batch.item_id,
                                            tbl_warehouse.dist_id";
//                    echo $qry_dhq_cb;exit;
                    // where reporting date = $to
                    // and dist_id in ()

                    $dhq_cb = array();
                    $qryResDhqCb = mysql_query($qry_dhq_cb);
                    if (mysql_num_rows(mysql_query($qry_dhq_cb)) > 0) {
	...
	itminfo_tab.itm_id IN (1,2,9,3,5,7,8,13)
                    ...
                    if (mysql_num_rows(mysql_query($qryitm)) > 0) {
                        ...
                        while ($row = mysql_fetch_array($qryResItm)) {
                            $row['generic_name'] = str_replace(array("\n", "\r"), '', $row['generic_name']);
                            $itm_arr[$row['itm_id']] = $row['generic_name'];
                            $itm_gen[$row['itm_id']] = $row['itm_name'];
                            $generic_arr[$row['itm_name']][$row['itm_id']] = $row['generic_name'];
                            $generic_arr2[$row['itm_name']] = $row['itm_name'];
                        }
                    }
...
//                   print_r($generic_arr);
//                   print_r($dist_cb2);
//                   print_r($dhq_issuance2);
//                   print_r($dhq_cb2);
//                   exit;
                    ?>
                    <table width="100%" >
                        <tr>
                            <td align="center"><h4 class="center"> Provincial FP Stock Status <br>
                        ...
                        <thead>
                            ...
                            <tr class = "bg-success">
                                ...
                                foreach ($dist_arr as $distid => $dist_name) {
                                    ...
                                    echo '<th align="center" width=\"50\">DHQ MOS</th>';
...
//                                echo '<td>'.@$dhq_cb2[$distid][$itmid].'/('.@$dhq_issuance2[$distid][$itmid].'/12):'.$dhq_mos.'</td>';
                                echo '<td align="right" title="' . @$dhq_cb2[$distid][$itmid] . '/(' . @$dhq_issuance2[$distid][$itmid] . '/12):">' . (($dhq_mos == 0) ? '0' : number_format($dhq_mos, 2)) . '</td>';
