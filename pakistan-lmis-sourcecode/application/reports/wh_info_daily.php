...
//Checking month
    //Getting month
    $where1 .=" tbl_hf_data_daily.reporting_date = '" . $_REQUEST['year'] . "-" . $_REQUEST['month'] . "-" . $_REQUEST['date'] . "' ";

//Checking whid
if (isset($_REQUEST['whId']) && $_REQUEST['whId'] != "") {
    //Getting whid
    $where1 .=" AND tbl_hf_data_daily.warehouse_id = " . $_REQUEST['whId'] . " ";
}

$query_xmlw = "SELECT * FROM ( 
			 (
					SELECT
						tbl_warehouse.wh_id,
						tbl_warehouse.wh_name,
						MONTH (tbl_hf_data_daily.reporting_date) AS report_month,
						YEAR (tbl_hf_data_daily.reporting_date) AS report_year,
						itminfo_tab.itm_name,
						itminfo_tab.frmindex,
						tbl_hf_data_daily.opening_balance AS wh_obl_a,
						tbl_hf_data_daily.received_balance AS wh_received,
						tbl_hf_data_daily.issue_balance AS wh_issue_up,
						tbl_hf_data_daily.adjustment_positive AS wh_adja,
						tbl_hf_data_daily.adjustment_negative AS wh_adjb,
						tbl_hf_data_daily.closing_balance AS wh_cbl_a
					FROM
						tbl_warehouse
					INNER JOIN tbl_hf_data_daily ON tbl_hf_data_daily.warehouse_id = tbl_warehouse.wh_id
					INNER JOIN itminfo_tab ON tbl_hf_data_daily.item_id = itminfo_tab.itm_id
					WHERE 
						$where1 ";
     
        //$query_xmlw .= "   AND itminfo_tab.itm_category = 1 ";
        $query_xmlw .= "
				)) A
				ORDER BY A.frmindex ASC";
//Query result
        if(!empty($_REQUEST['qry']) && $_REQUEST['qry'] == 'yes'){
            echo $query_xmlw;
        }
...
$title = $_REQUEST['date'].' '.date('F', mktime(0, 0, 0, $_REQUEST['month'])) . ' ' . $_REQUEST['year'] . " Report For Warehouse: '" . $whInfo['wh_name'] . "' Stakeholder: '" . $whInfo['stkname'] . "' District: '" . $whInfo['District'] . "' And Province: '" . $whInfo['Province'] . "'";
?>
