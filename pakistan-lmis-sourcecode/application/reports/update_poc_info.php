...
$dist_id=mysql_real_escape_string($_REQUEST['dist_id']);
$stk_id=mysql_real_escape_string($_REQUEST['stk_id']);
$userSql = "SELECT
...
if (isset($_REQUEST['submit']) && !empty($_REQUEST['submit'])) {
    ...
    if ($button == 'contact') {
        $sysusr_name = mysql_real_escape_string($_POST['name']);
        $sysusr_deg = mysql_real_escape_string($_POST['office']);
        $sysusr_email = mysql_real_escape_string($_POST['email']);
        $sysusr_dept = mysql_real_escape_string($_POST['department']);
        $sysusr_cell = mysql_real_escape_string($_POST['cellnumber']);        
        $created_date = $modified_date = date("Y-m-d H:i:s");

        $strSqlUpd = "UPDATE sysuser_tab SET sysusr_name = '$sysusr_name', sysusr_email = '$sysusr_email' , sysusr_deg = '$sysusr_deg' ,sysusr_dept = '$sysusr_dept' ,sysusr_cell = '$sysusr_cell',sysusr_ph = '$sysusr_cell' 
 
WHERE
		sysuser_tab.UserID = $u_id
        limit 1
 ";
//        print_r($strSqlUpd);exit;
