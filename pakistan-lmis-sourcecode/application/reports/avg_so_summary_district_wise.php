...
if (isset($_REQUEST['submit_btn'])) {
    ...
    $dist = (!empty($_REQUEST['district']) ? $_REQUEST['district'] : '');
}
$where_clause = "";

if (!empty($province))
    $where_clause .= " AND avg_stock_out_summary_district_wise.province_id in (" . $province . ")  ";
if (!empty($stk))
    $where_clause .= " AND avg_stock_out_summary_district_wise.stakeholder_id in (" . $stk . ")  ";

$where_item = '';
if (!empty($itm))
    $where_item = " AND avg_stock_out_summary_district_wise.product_id in (" . $itm . ")";
...
//query result
                ...
                if (isset($_REQUEST['submit_btn'])) {
                    ...
                    $qry_1 = "  
                    SELECT
avg_stock_out_summary_district_wise.province_id AS prov_id,
avg_stock_out_summary_district_wise.product_id AS itm,
avg_stock_out_summary_district_wise.total_facilities AS totalWH,
avg_stock_out_summary_district_wise.reporting_month,
avg_stock_out_summary_district_wise.reported_facilities,
avg_stock_out_summary_district_wise.disabled_facilities,
avg_stock_out_summary_district_wise.dormant_facilities,
avg_stock_out_summary_district_wise.so_facilities,
avg_stock_out_summary_district_wise.province_name LocName,
avg_stock_out_summary_district_wise.product_name,
avg_stock_out_summary_district_wise.district_name
FROM
avg_stock_out_summary_district_wise
WHERE
avg_stock_out_summary_district_wise.reporting_month BETWEEN '" . $from_date . "' AND '" . $to_date . "'
$where_clause
AND avg_stock_out_summary_district_wise.product_id IN ($itm) AND avg_stock_out_summary_district_wise.district_id IN ($dist) ";
//                    echo $qry_1;exit;
                                ...
                                <div class="col-md-12">
                                    <h4 class="page-title1   center"> Reporting Period : <?php echo date('M-Y', strtotime($from_date)); ?> To <?php echo date('M-Y', strtotime($to_date)); ?></h3>
                                        <h4 class="page-title1 row  center"> 
                                            <div class=" col-md-11">
                                                <?php echo $dd; ?> - <?php echo implode(',', $stk_name); ?> 
                                            </div>
                                            ...
                                            <table id="AvgStockOut" name="tbl" class="table table-bordered table-condensed" border="">    
                                                <tr>
                                                    ...
                                                    foreach ($months_list as $k => $v) {
                                                        echo '<th colspan="5" class="months_td" style="text-align:center !important;">' . $dd; ?> - <?php echo implode(',', $stk_name) . "-" .date('M Y', strtotime($v)) . '</th>';
                                                    }
                                                    ?>
                        ...
                        <div class="col-md-12">
                            <div class="widget widget-tabs">
                                <div class="widget-body">
                                    ...
                                    <?php
                                    //xml for chart
                                    $xmlstore = '<chart   caption="Average Stock Out at SDP\'s for reporting period : ' . date('M-Y', strtotime($from_date)) . ' - ' . $dd . ' " showvalues="1" yaxismaxvalue="1"  subcaption="" xaxisname="Products" exportEnabled="1"  yaxisname="Percentage" numberprefix="" theme="fint">';
                                    $xmlstore .= ' <categories>';
                                    foreach ($months_list as $k => $v) {
...
//include footer
    ...
    <script>
          $(function() {
            ...
            $('#province, #stakeholder').change(function(e) {
                $('#district').html('<option value="">All</option>');
                $('#warehouse').html('<option value="">Select</option>');
                showDistricts($('#province').val(), $('#stakeholder').val());
            });
            $('#stakeholder').change(function(e) {
                $('#warehouse').html('<option value="">All</option>');
            });

            $(document).on('change', '#province, #stakeholder, #district', function() {
