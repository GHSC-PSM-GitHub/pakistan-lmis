...
{
    $where_clause .=" AND tbl_locations.PkLocID IN (".$_REQUEST['district'].") ";
}
...
//            echo $qry_1;exit;
                $res_1 = mysql_query($qry_1);
                $distict = $total_sdps= $rep_start_months= array();
                while($row_1 = mysql_fetch_array($res_1))
                {
                    $distict[$row_1['PkLocID']] = $row_1['LocName'];
...
//                            foreach($distict as $dist_id => $wh_name ){
//                                echo $dist_id;
//                                echo $wh_name;
                                $total_reported = $total_active= array();
                                foreach($distict as $dist_id => $dname ){
                                foreach($total_sdps as $wh_id => $wh_name ){
                                    ...
                                    foreach($months_list as $k => $month ){
                                        ...
                                        if( !empty($reporting_wh_arr[$wh_id][$month])   ){
                                            ...
                                            @$total_reported[$dist_id][$month]++;
                                            @$total_active[$dist_id][$month]++;
                                        }
...
//                            Print_r($rep_percentage);
//                            echo "<hr>";
//                            var_dump($rep_percentage);
...
if(isset($_REQUEST['district']) && $_REQUEST['district'] != 'All')
{
    $chart_data .= ' <dataset seriesname="'.$dist_arr[$_REQUEST['district']].' Reporting Rate">';
    foreach($months_list as $k => $month)
    {   
    ...
    foreach($dist_arr as $did => $dname)
    $chart_data .= ' <dataset seriesname="'.$dname.' Reporting Rate">';
    foreach($months_list as $k => $month)
    {   
        $val=(!empty($rep_percentage[$month])? $rep_percentage[$month]:'0');
        $chart_data .= '    <set  value="'.number_format($val,2).'"  />';
...
//                                $total_reported = $total_active= array();
                                foreach($total_sdps1 as $wh_id => $wh_name ){
                                    ...
                                    foreach($months_list as $k => $month ){
                                        ...
                                        {
                                            $rep_percentage[$month] = 100*(!empty($total_reported[$month])?$total_reported[$month]:'0') / $total_active[$month];
                                            echo '<td align="center">'.number_format($rep_percentage[$month],2).'</td>';
                                        }else{
                                            $rep_percentage[$month] = 0;
                                            echo '<td align="center">'.number_format($rep_percentage[$month],2).'</td>';
                                        }
                        ...
                        $c=1;
                                ...
                                foreach($total_sdps1 as $wh_id => $wh_name ){
                                    ...
                                    foreach($months_list as $k => $month ){
                                        ...
                                        {
                                            $rep_percentage[$month] = 100*(!empty($total_reported[$month])?$total_reported[$month]:'0') / $total_active[$month];
                                            $xmlstore .= "<cell>".number_format($rep_percentage[$month],2)."</cell>";
                                        }else{
                                            $rep_percentage[$month] = 0;
                                            $xmlstore .= "<cell>".number_format($rep_percentage[$month],2)."</cell>";
                                        }
