                    ...
                    <option value="">Select</option> <?php
                       $queryprov = "SELECT
                        ...
                        while ($rowprov = mysql_fetch_array($rsprov)) {
                            $sel = "";
                            if (!empty($_POST['prov_sel']) && $_POST['prov_sel'] == $rowprov['prov_id']) {
                                $sel = "selected='selected'";
                                $prov_name = $rowprov['prov_title'];
                            }elseif($_SESSION['user_province1'] == $rowprov['prov_id']){
                                $sel = "selected='selected'";
                                $prov_name = $rowprov['prov_title'];
                            }
                            //Populate prov_sel combo
                            ?>
                            <option value="<?= $rowprov['prov_id'];?>" <?= $sel;?>><?= $rowprov['prov_title'];?></option>
                    <?php
                        } 
                    ?>                        
                </select>
        ...
        <div class="control-group" id="districtsCol">
            ...
            <div class="controls">
                <select name="district" id="district"  class="form-control input-sm" required>
                    ...
                    <option value="all" <?= (!empty($_POST['district']) && $_POST['district'] == 'all')?'selected':'';?>>All</option>
                    <?php
                    //select query
                    //gets
                    //district id
                    if (!empty($_POST['prov_sel']) && $_POST['prov_sel'] != '') {
                        $provinceCondition = " AND tbl_locations.parentid = ".$_POST['prov_sel']." ";
                    }else{
                        $provinceCondition = " AND tbl_locations.parentid = ".$_SESSION['user_province1']." ";
                    }
                    //district name
                    $queryDist = "SELECT
                    ...
                    while ($rowDist = mysql_fetch_array($rsDist)) {
                        $sel = "";
                        if (!empty($_POST['district']) && $_POST['district'] == $rowDist['PkLocID']) {
                            $sel = "selected='selected'";
                            $dist_name = $rowDist['LocName'];
                        }elseif($_SESSION['user_district'] == $rowDist['PkLocID']){
                            $sel = "selected='selected'";
                            $dist_name = $rowDist['LocName'];
                        }
                        //populate district combo
                        ?>
                        <option value="<?= $rowDist['PkLocID'];?>" <?= $sel;?>><?= $rowDist['LocName'];?></option>
                    <?php
                    }
                    ?>
                </select>
