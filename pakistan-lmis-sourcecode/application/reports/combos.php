 ...
 * 
 */
$showHFTypeArr = array('pwd3','sdp_p', 'spr2','pwd3y', 'spr2y', 'dpw_punjab', 'clr11y','outletp','spr10');
?>
<script>
	$(function() {
		<?php 
	...
	function showDistricts()
	{
		...
		if (provinceId != '')
		{
			...
			$.ajax({
				url: 'ajax_calls.php',
				data: {provinceId: provinceId, dId: '<?php echo $districtId; ?>', stkId: stkk,allOpt:'<?=(!empty($allOpt)?$allOpt:'')?>',rptId:'<?=(!empty($rptId)?$rptId:'')?>',<?=((!empty($rptId) && $rptId=='spr10')?'validate:\'no\'':'')?>},
				type: 'POST',
