#####################
Batch Text Replacer Demo ID:472025
#####################
<?php
...
//for dtl process
    $dtl_wh_arr=array(96574,96575,96576);
    $test_result=$objStockDetail->find_by_stock_id($stockid);
    while ($row = mysql_fetch_array($test_result)) {
        $batch_array[$row['BatchID']]=$row['BatchID'];
        $wh_to_array[$row['WHIDTo']]=$row['WHIDTo'];
    }
    foreach ($wh_to_array as $key => $value) {
        if(in_array($value, $dtl_wh_arr))
        {
            foreach ($batch_array as $key => $value) {
                $objStockBatch->changestatus($key,"Stacked");
            }
    ...
    // result is the info of warehouse for which the stock is issued.
    $result = $objStockMaster->getWhLevelByStockID($stockid);
//    echo '<pre>';print_r($_REQUEST);print_r($result);exit;
   
    // for lvl 7 , if IM enabled then do NOT auto recieve
    if ($result['im_enabled'] != '1') {
       
        if ($result['level'] == 7 || $result['level'] == 9) {
            $objStockMaster->autoReceiveData($stockid);

            //checking if the vouchers is completely recieved
            if ($objStockMaster->auto_rcv_is_pending($stockid)) {
                $objStockMaster->autoReceiveData($stockid);
            }
        }
...
?>
#####################
Batch Text Replacer Demo ID:0023633
#####################
