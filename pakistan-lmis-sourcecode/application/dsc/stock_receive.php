#####################
Batch Text Replacer Demo ID:72508001
#####################
<?php
/**
 * stock_receive
...
if($_SESSION['user_role']==69){
    redirect('stock_receive_readonly.php');
    exit;
}
//get warehouse id
...
if($_SESSION['user_stakeholder1'] != 1)
    $items = $objManageItem->GetAllProduct_of_stk($stk,true);
else
...
if (!empty($_REQUEST['product'])) {
    ...
    $manufacturers = $manufacturer_product = $objstk->GetManufacturersOfProd($_REQUEST['product']);
            ...
            <div class="page-content"> 
                                                    ...
                                                    <div class="controls">
                                                        <select name="manufacturer" id="manufacturer" class="form-control input-medium">
                                                            ...
                                                            if (mysql_num_rows($manufacturers) > 0) {
                                                                while ($row = mysql_fetch_object($manufacturers)) {
                                                                    ?>
                                                                    <option value="<?php echo $row->stk_id; ?>" <?php if ($_REQUEST['manufacturer'] == $row->stk_id) { ?> selected="" <?php } ?>><?php echo $row->stkname; ?></option>
                                                                    <?php
                                                                }
                                                            }
                                                            ?>
                ...
                <div class="widget-body"><img src="../../public/images/excel-16.png" <i id="btnExport" onclick="fnExcelReport();" class="fa fa-file-excel-o" style="font-size:32px;color:#FF0000;float: right;"></i>
                            ...
                            <div class="widget-body"> 
                                ...
                                <!-- Table --> 
                                <!-- Table -->
                                <table class="receivesearch table table-bordered table-condensed" id="example">
                                        ...
                                        if ($result != FALSE) {
                                            ...
                                            while ($row = mysql_fetch_object($result)) {
                                                if ($transNo != $row->PkStockID) {
                                                    $bgColor = (!empty($bgColor) &&  $bgColor == '#CCC') ? '#FFF' : '#CCC';
                                                } else {
                                                    $bgColor = $bgColor;
                                                }
                                                ...
                                                <tr class="gradeX" style="background-color:<?php echo $row->field_color; ?> !important" id="<?php echo $row->PkDetailID; ?>">
                                                    ...
                                                    <td width="8%"><?php echo date("d-M-y", strtotime($row->TranDate)); ?></td>
                                                    ...
                                                    <td width="8%"><?php echo date("d-M-y", strtotime($row->batch_expiry)); ?></td>   
                                                    <td><?php echo date("d-M-y", strtotime($row->CreatedOn)); ?></td>   
                                                    
                                                    <td class="text-center">
                                                    <?php
//                                                    if ($_SESSION['user_role'] == 4 || $_SESSION['user_role'] == 38 || $_SESSION['user_role'] == 54) {
...
#####################
Batch Text Replacer Demo ID:561505
#####################
