...
//                print_r($qry);exit;
                ...
                <table class='table table-bordered table-condensed'>
                    <?php while ($row = mysql_fetch_assoc($qryRes)) { ?>
                        ...
                        <tr><th>Quantity</th><td class="bold"><?php echo number_format($row['Qty']) ?>
                                <span class="pull-right">
                                    <a href="<?=APP_URL?>developer_reports/fix_stock_batch_redirect.php?id=<?=$batch_id?>&redirect=dsc/product-ledger-history">*</a>
                                </span></td></tr>
                ...
                $qry_trail = "SELECT tbl_stock_master.WHIDFrom,
                               ...
                               LEFT JOIN stakeholder ON tt.stkid = stakeholder.stkid
...
INNER JOIN tbl_trans_type ON tbl_stock_detail.adjustmentType = tbl_trans_type.trans_id
                               WHERE stock_batch.batch_id='$batch_id'
                               ORDER BY
                                       DATE_FORMAT(tbl_stock_master.TranDate,'%Y-%m-%d') ASC,tbl_stock_detail.PkDetailID ";
//                echo $qry_trail;exit;
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <?php
                                        ...
                                        if($row['TranTypeID']==1)$file_name = 'printReceive';
                                        elseif($row['TranTypeID']==2)$file_name = 'printIssue';
                                            
                                    ?>
                                    <td><a  onclick="window.open('<?=$file_name?>.php?id=<?php echo $row['PkStockID']; ?>', '_blank', 'scrollbars=1,width=842,height=595');" href="javascript:void(0);"><?php echo $row['TranNo'] ?></a></td>
                                    ...
                                    <td><?php if ($row['adjustmentType']==1) {echo 'Receiving';}elseif ($row['adjustmentType']==2) {echo 'Issuance';}else{echo 'adjustment';} ?></td>
