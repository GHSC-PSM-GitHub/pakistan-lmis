#####################
Batch Text Replacer Demo ID:8759765891
#####################
<?php
/**
 * gatepass_ajax
...
if (isset($_POST['vehicleType'])) {
    ...
    $qry = "SELECT
			...
			FROM
				gatepass_vehicles
			WHERE
				gatepass_vehicles.vehicle_type_id = $vehicleType  AND wh_id = '".$_SESSION['user_warehouse']."'";
    //query result
					...
					SELECT
						...
						IFNULL(
							SUM((
								...
								WHERE
									gatepass_detail.stock_detail_id = tbl_stock_detail.PkDetailID AND
                                                                        gatepass_master.gp_status <> 'deleted'
							)),
							0
						) AS gatepassQty
					FROM
				...
				(
					...
					WHERE
						gatepass_detail.stock_detail_id = tbl_stock_detail.PkDetailID
                                                  AND gatepass_master.gp_status <> 'deleted'
					), NULL, 0)
				) AS Qty
			FROM
				tbl_stock_master
...
#####################
Batch Text Replacer Demo ID:764958313
#####################
