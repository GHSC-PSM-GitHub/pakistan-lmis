#####################
Batch Text Replacer Demo ID:8192338
#####################
<?php
/**
 * stock_issue
...
if($_SESSION['user_role']==69){
    redirect('stock_issue_readonly.php');
    exit;
}
//get warehouse id
                    ...
                    <div class="col-md-12">
                                                        ...
                                                        if ($stakeholders != FALSE) {
                                                            ...
                                                            while ($row = mysql_fetch_object($stakeholders)) {
                                                                ...
                                                                if($_SESSION['user_stakeholder1'] == 145 )
                                                                    if( $row->stkid !=2 && $row->stkid !=73 ) continue;
                                                                ?>
                        ...
                        <div class="widget" data-toggle="collapse-widget">
                            <div class="widget-head">
                             ...
                             <div class="widget-body"><img src="../../public/images/excel-16.png" <i id="btnExport" onclick="fnExcelReport();" class="fa fa-file-excel-o" style="font-size:32px;color:#FF0000;float: right;"></i>
                            ...
                            <div class="widget-body"> 
                                ...
                                <!-- Table -->
                                <table class="issuesearch table table-bordered table-condensed" id="example">
                                        ...
                                        if ($result != FALSE) {
                                            ...
                                            while ($row = mysql_fetch_object($result)) {
                                                ...
                                                <tr class="gradeX" style="background-color:<?php echo $row->field_color; ?> !important" id="<?php echo $row->PkDetailID; ?>">
                                                    ...
                                                    <td><?php
                                $pop = 'onclick="window.open(\'product-ledger-history.php?id=' .$row->BatchID . '\',\'_blank\',\'scrollbars=1,width=840,height=595\')"';
...
//                                echo $b['batch_no']; 
                                ?>
                                                    ...
                                                    <td class="text-right"><?php echo (!empty($price)?number_format($price):''); ?></td>
                                                    <td><?php echo date("d-M-y", strtotime($row->batch_expiry)); ?></td>  
                                                    <td><?php echo date("d-M-y", strtotime($row->CreatedOn)); ?></td>
                                                    <?php
                                                     echo '<td class="text-center" dev_vals="sess_u_lvl:'.$_SESSION['user_level'].',sess_usr_id:"'.$_SESSION['user_id'].'",created_by:"'.$row->CreatedBy.'",row_lvl:"'.$row->lvl.'",row_is_rcvd:"'.$row->IsReceived.'",row_is_im:"'.$row->is_allowed_im.'",row_createdOn:"'.$row->CreatedOn.'",im_start:"'.$row->im_start_month.'">';
                                                    if ($_SESSION['user_level']<= 3 && $_SESSION['user_id'] == $row->CreatedBy ) {
                                                        //national and prov level only, can delete only vouchers which have not been recieved Yet.
                                                        if ($row->lvl < 7 && $row->IsReceived == 0) {
                                                        ...
                                                        elseif($row->lvl >= 7 ){
                                                            if($row->is_allowed_im == 0){
                                                                echo "<a href=\"javascript: void(0)\" onClick=\"deleteIssuanceAndReceiving(" . $row->PkDetailID . ", " . $row->BatchID . ")\"><img src=\"" . PUBLIC_URL . "images/cross.gif\" class=\"cursor\"></a>";
                                                            }
                                                            elseif($row->is_allowed_im == 1 && $row->CreatedOn < $row->im_start_month && $row->CreatedOn >= '2018-11-01'){
                                                                echo "<a href=\"javascript: void(0)\" onClick=\"deleteIssuanceAndReceiving(" . $row->PkDetailID . ", " . $row->BatchID . ")\"><img src=\"" . PUBLIC_URL . "images/cross.gif\" class=\"cursor\"></a>";
                                                            }
...
#####################
Batch Text Replacer Demo ID:7079277745
#####################
