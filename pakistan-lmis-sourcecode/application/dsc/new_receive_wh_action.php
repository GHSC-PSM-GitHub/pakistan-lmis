#####################
Batch Text Replacer Demo ID:026743277
#####################
<?php
...
//check stock id
    if (isset($_REQUEST['stock_id']) && !empty($_REQUEST['stock_id'])) {
        ...
        if (mysql_num_rows($stockDetail) > 0) {
            ...
            $date_breakdown = explode('/',$rec_date);
            ...
            //get Last ID
            $last_id = $objStockMaster->getLastID(date('Y-m-01',strtotime($issue_date_formatted)), date('Y-m-t',strtotime($issue_date_formatted)), $type_id);
...
?>
#####################
Batch Text Replacer Demo ID:73898772
#####################
